...
class DQN(nn.Module):
    def __init__(self, in_states, in_actions):
        ...
        self.fc1 = nn.Linear(in_states + in_actions, 64, dtype=torch.float64)   # first fully connected layer
        # self.fc2 = nn.Linear(128, 32, dtype=torch.float64)   # first fully connected layer
        self.out = nn.Linear(64, 1, dtype=torch.float64) # ouptut layer w
...
class Agent():
    def __init__(self, env):
        ...
        self.network_sync_rate = 50          # number of steps the agent takes before syncing the policy and target network
        self.replay_memory_size = 5000       # size of replay memory
    ...
    def train(self, episodes):
        ...
        # Save policy
        torch.save(self.policy_dqn.state_dict(), f"model_maze{self.env.maze[4]}.pt")
        ...
        # Save plots
        plt.savefig(f'graph_maze{self.env.maze[4]}.png')
    ...
    def test(self, episodes):
        ...
        policy_dqn.load_state_dict(torch.load(f"model_maze{self.env.maze[4]}.pt"))
