...
class Grid():
    ...
    def get_env_state(self):
        # player grid
        player_state = np.zeros((ROWS, COLUMNS))
        player_row, player_col = self.get_player_pos()
        player_state[player_row][player_col] = 1

        # goal grid
        goal_state = np.zeros((ROWS, COLUMNS))
        goal_state[self.goal_row][self.goal_col] = 1

        # wall grid
        wall_state = np.zeros((ROWS, COLUMNS))
...
if __name__ == '__main__':
    env = Grid('maze1', show_render=True)
    env.reset()
