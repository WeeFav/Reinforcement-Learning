...
class Agent:
    ...
    def select_action(self, state, eps):
        ...
        if p <= eps:
            ...
            preds = torch.tensor([preds1, preds2, preds3, preds4])
            print(preds)
            mean, std, var = torch.mean(preds), torch.std(preds), torch.var(preds)
            preds = (preds - mean) / std
            print("up: {}, down: {}, left: {}, right: {}".format(preds[0].detach().numpy(), preds[1].detach().numpy(), preds[2].detach().numpy(), preds[3].detach().numpy()))
            self.action = torch.argmax(preds)
...
def train(V):
    ...
    pref_l = [0]*2
    eps = [1, 0.3, 1, 0.3, 1, 0.3, 1, 0.3, 1, 0.3, 1, 0.3, 1, 0.3, 1, 0.3, 1, 0.3, 1, 0.3, 1, 0.3, 1, 0.3]
    ...
    for i in range(50):
        ...
        while steps < 500:
            ...
            while not done and steps < 500:
                print("step: ", steps)
                steps += 1
                action = agent.select_action(env.get_state(), eps[i])
                agent.replay_buffer.append((env.get_state(), action))
                done = env.step(action, 10)
